...
size_t a_start = delta_start + N - 1; // trying N-2 instead of N-1....
...
//    const double w_throttle_steer = 10;
    ...
    // Minimize throttle with high steering angle
//    for (int t = 0; t < N - 1; t++) {
...
//      AD<double> psides0 = CppAD::atan(coeffs[1]);
      ...
      // polynomial of the waypoints line dependent on number of coefficients
      AD<double> f0 = 0.0;
      for (int i=0; i<coeffs.size(); i++){
        f0 += coeffs[i] * CppAD::pow(x0, i);
      }
      
      // the desired heading of the car dependent on number of coefficients
...
MPC::Results MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  //size_t i;
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_states = 6; // set the number of states
  size_t n_actuators = 2; // set the number of actuators
  size_t n_vars = n_states * N + n_actuators * (N - 1); // set number of variables
  ...
  // TODO: Tweak these parameters for throttle max and min
  ...
  // Sets constraint lowerbound to initial state
  ...
  // Sets constraint upperbound to initial state
  ...
  // variables to return the predicted path points for display as a green line
  vector<double> mpc_x = {};
  vector<double> mpc_y = {};
  ...
  result.MPC_X = mpc_x;
  result.MPC_Y = mpc_y;
  result.ACTUATORS = {solution.x[delta_start],   solution.x[a_start]};
